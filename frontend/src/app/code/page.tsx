...
export default function CodePage() {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <h1 className="text-xl font-semibold text-slate-100">Code analysis</h1>
      ...
      <div className="mt-4 flex gap-2">
        <input
          ...
          className="flex-1 rounded-lg border border-slate-800 bg-slate-900/60 px-3 py-2 text-sm text-slate-200 outline-none focus:border-slate-600"
        />
        <button
          onClick={run}
            ...
            <ul className="space-y-1 text-xs">
              {result.signals.map((s, i) => (
                <li key={i} className="flex gap-3">
                  ...
                  <span className="rounded border border-slate-700 bg-slate-800 px-1.5 text-[10px] text-slate-300">
                    {s.category}
                  </span>
