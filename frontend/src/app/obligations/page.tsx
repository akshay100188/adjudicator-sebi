...
export default function ObligationsPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <h1 className="text-xl font-semibold text-slate-100">Obligation browser</h1>
      ...
      <select
        ...
        className="mt-4 rounded border border-slate-800 bg-slate-900/60 px-3 py-1.5 text-sm text-slate-300"
      >
        {chapters.map((c) => (
      ...
      <ul className="mt-4 space-y-2">
        {shown.map((o) => (
          <li key={o.obligation_id} className="rounded-lg border border-slate-800 bg-slate-900/40">
            <button onClick={() => toggle(o.obligation_id)} className="flex w-full items-center gap-3 p-3 text-left">
              ...
              <span className="flex-1 text-sm text-slate-200">{o.title}</span>
              {o.valid_today && (
                <span className="rounded-full border border-emerald-800 bg-emerald-900/40 px-2 py-0.5 text-[10px] text-emerald-300">
                  valid today
                </span>
              )}
            </button>
              ...
              <div className="border-t border-slate-800 p-4 text-xs">
                {!detail ? (
                  ...
                  <div className="space-y-3">
                    <p className="text-slate-300">{detail.obligation_text}</p>
                    ...
                    <div>
                      <div className="text-slate-500">
                        Source: <span className="font-mono text-emerald-400">{detail.source_circular_ref}</span> ·
                        clauses {(detail.clause_refs || []).join(", ")}
                      </div>
                    </div>
                    <div>
                      <div className="text-slate-500">Citation graph (supersession / consolidation)</div>
                      <ul className="ml-4 mt-1 space-y-0.5 font-mono text-slate-400">
                        {detail.citation_graph.map((e, i) => (
                          <li key={i}>
                            {e.relation_type} → {e.to_ref} <span className="text-slate-600">(depth {e.depth})</span>
                          </li>
                        ))}
                      </ul>
                    </div>
